...
export class ConfigState {
  ...
  getConfig({ setState }: StateContext<Config>) {
    ...
    // Configuration to change the folders view
    const featuresViewFolder = localStorage.getItem('featuresView.folder');
    if (featuresViewFolder !== null) {
      if (['tiles','list'].includes(featuresViewFolder)) {
        configFile.featuresView.folder = featuresViewFolder;
      } else {
        localStorage.removeItem('featuresView.folder');
      }
    }
