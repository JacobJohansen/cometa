...
export class ResultsState {
     ...
     * @author Alex Barba
     */
    verifyAndFixMainKeys(state: IResults, feature_id: number, run_id: number, browser_info: BrowserstackBrowser, status?: string) {
        const browserKey = getBrowserKey(browser_info);
        // Check feature id
    ...
    setFeatureInitializing(
        { setState }: StateContext<IResults>,
        { feature_id, run_id, browser_info}: WebSockets.FeatureInitializing
    ) {
        this.resetTimeout(feature_id, run_id, browser_info);
        setState(
            produce((ctx: IResults) => {
                this.verifyAndFixMainKeys(ctx, feature_id, run_id, browser_info, 'Initializing');
    ...
    setFeatureFinished(
        ...
        setState(
            produce((ctx: IResults) => {
                this.verifyAndFixMainKeys(ctx, feature_id, run_id, browser_info, 'Completed');
    ...
    setStepStarted(
        { setState }: StateContext<IResults>,
        { feature_id, run_id, browser_info, step_name, step_index, datetime }: WebSockets.StepStarted
    ) {
        this.clearTimeout(feature_id, run_id, browser_info);
        setState(
            produce((ctx: IResults) => {
                this.verifyAndFixMainKeys(ctx, feature_id, run_id, browser_info, step_name);
    ...
    setStepFinished(
        { setState }: StateContext<IResults>,
        { feature_id, run_id, browser_info, step_index, step_name, datetime, step_result_info, step_time, error, screenshots }: WebSockets.StepFinished
    ) {
        this.clearTimeout(feature_id, run_id, browser_info);
        setState(
            produce((ctx: IResults) => {
    ...
    setFeatureStarted(
        { setState }: StateContext<IResults>,
        { feature_id, run_id, browser_info, start_at }: WebSockets.FeatureStarted
    ) {
        const browserKey = getBrowserKey(browser_info);
        this.resetTimeout(feature_id, run_id, browser_info);
        setState(
            produce((ctx: IResults) => {
                this.verifyAndFixMainKeys(ctx, feature_id, run_id, browser_info, 'Starting');
    ...
    setFeatureError(
        { setState }: StateContext<IResults>,
        { feature_id, run_id, browser_info, error }: WebSockets.FeatureError
    ) {
        this.clearTimeout(feature_id, run_id, browser_info);
        const browserKey = getBrowserKey(browser_info);
        setState(
            produce((ctx: IResults) => {
                try {
                    this.verifyAndFixMainKeys(ctx, feature_id, run_id, browser_info, error);
                    ctx[feature_id].running = false;
