...
export class FeatureActionsComponent implements OnInit {
  ...
  viewLog() {
    ...
    this._mixpanel.track('Show feature result log', { feature_result_id: featureResultId })
    if (!featureResultId) {
  ...
  toggleNotification() {
    ...
    if (notifications.includes(featureStore.feature_id)) {
      this._sharedActions.trackCheckboxAction('Toggle notifications', false)
      return new WebSockets.RemoveNotificationID(featureStore.feature_id);
    } else {
      this._sharedActions.trackCheckboxAction('Toggle notifications', true)
