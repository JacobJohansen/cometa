...
export class FeatureRunComponent {
  ...
  ngAfterViewInit() {
    ...
    FeatureRunComponent.avg += elapsed;
    console.log('C.[22] -- FeatureRun #', FeatureRunComponent.n++, ':', elapsed, "ms")
    console.log('C.[23] -- Average ', (FeatureRunComponent.avg / FeatureRunComponent.n))
  }

  openVideo(test: FeatureResult) {
    this._mixpanel.track('Show video')
  ...
  setResultStatus(result: FeatureResult, status: 'Success' | 'Failed' | '') {
    this._mixpanel.track('Override status of feature result')
  ...
  setRunStatus(run: FeatureRun, status: 'Success' | 'Failed' | '') {
    this._mixpanel.track('Override status of run')
