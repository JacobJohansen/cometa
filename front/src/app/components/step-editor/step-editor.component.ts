...
export class StepEditorComponent extends SubSinkAdapter implements OnInit {
  ...
  exportClipboard() {
    this._mixpanel.track('Export steps to Clipboard');
  ...
  export() {
    this._mixpanel.track('Export steps to file');
    exportToJSONFile(this.feature.feature_name, this.getSteps());
  }

  readJson(e) {
    this._mixpanel.track('Import steps from file');
  ...
  importClipboard() {
    this._mixpanel.track('Import steps from Clipboard');
