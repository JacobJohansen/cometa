...
export class NetworkPaginatedListComponent implements OnChanges {
  ...
  loadPage(page: number, size: number) {
    ...
    return this._http.get<PaginatedResponse<any>>(`${this._api.api}${this.endpointUrl}`, {
      ...
      tap(_ => this.startTime = this.getUnix())
    )
  }


  setEnd() {
    this.endTime = this.getUnix();
    // Better use a logger?
    // Only do console.logs on Development ... do not on PROD!!!
    // Use "C.[number]" to identify where this console.log comes from use "C." to be cler that this is intended behaviour
    console.log('C.[123] List time:', (this.endTime - this.startTime)+'ms')
  }
