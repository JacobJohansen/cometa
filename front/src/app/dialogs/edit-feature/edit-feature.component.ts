...
export class EditFeature implements OnInit, OnDestroy {
  ...
  editOrCreate() {
    ...
    if (this.data.mode === 'edit' || this.data.mode === 'clone') {
      ...
      // Create feature XHR
      dataToSend.cloud = 'browserstack';
      if (this._browserSelection) {
        dataToSend.cloud = this._browserSelection.testing_cloud.value;
      }
      this.saving$.next(true);
      this._api.createFeature(dataToSend, {
        loading: 'translate:tooltips.creating_feature'
      }).pipe(
        finalize(() => this.saving$.next(false))
      ).subscribe(res => {
        if (res.feature_id) {
          ...
          this.dialogRef.close(dataToSend);
          // Move to current folder
          this.moveFeatureToCurrentFolder(res.feature_id).pipe(
            switchMap(_ => {
              return this._store.dispatch([
                new Features.UpdateFeatureOffline(res),
                new Features.PushNewFeatureId(res.feature_id)
              ]);
            })
          ).subscribe();
        }
