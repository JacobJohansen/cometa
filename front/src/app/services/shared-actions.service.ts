...
export class SharedActionsService {
  ...
  handleSetting(featureId: number, type: string, event: MatCheckboxChange) {
    switch (type) {
      case 'need_help':
        if (event.checked) this._mixpanel.track('Ask for help');
  ...
  moveFolder(folder: Folder) {
    this._mixpanel.track('Move folder', { folder: folder })
    this._dialog.open(MoveItemDialog, {
      data: {
        type: 'folder',
        folder
  ...
  async run(featureId: number) {
    this._mixpanel.track('Run feature');
  ...
  openEditFeature(featureId: number = null, mode: 'edit' | 'clone' | 'new' = 'new') {
    if (mode === 'edit' || mode === 'clone') {
      if (mode === 'edit') {
        this._mixpanel.track('Modify feature', { feature_id: featureId });
      } else {
        this._mixpanel.track('Clone feature', { feature_id: featureId });
      }
      // Edit / Clone mode
  ...
  deleteFeature(featureId: number) {
    this._mixpanel.track('Delete feature', { feature_id: featureId });
    const feature = this._store.selectSnapshot<Feature>(CustomSelectors.GetFeatureInfo(featureId))
  ...
  openHTMLDiff(stepResultId: number) {
    ...
    this.loadingObservable(
      ...
      this._mixpanel.track('View HTML Difference')
  ...
  trackCheckboxAction(name: string, checked: boolean) {
    this._mixpanel.track(name, { enabled: checked });
  }
