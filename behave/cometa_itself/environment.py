...
def before_all(context):
    ...
    # Screenshot images are saved in /<screenshots_dir>/<feature_id>/<feature_run>/<feature_result_id>/<rand_hash>/<step_result_id>/$PREFIX<current|style|difference>.png
    # Templates images are saved in /<screeshots_dir>/templates/<feature_id>/<browser_key>/$PREFIXtemplate_<step_index>.png
    run_id = os.environ['feature_run']
    # Retrieve run hash
    ...
    logger.info("Trying to get a browser context on %s" % command)
...
""" % stdout.replace("\n\n", "\n")
    ...
    # se the headers for the request
...
def before_step(context, step):
    ...
    # Throws exception on Client01 testcase to add filter in QS page
    # FIXME or DELETE ME
    # if context.cloud == "local":
    #     try:
