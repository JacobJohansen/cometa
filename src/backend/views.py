...
COPYRIGHT = getattr(secret_variables, 'COMETA_COPYRIGHT', '')
NOREPLY_EMAIL = getattr(secret_variables, 'COMETA_NOREPLY_EMAIL', '')
...
def userDetails(request, *args, **kwargs):
    ...
    if apps.is_installed('extensions.payments'):
        from extensions.payments.util import get_requires_payment, get_user_usage_money
    ...
    if feature.need_help:
        ...
        try:
            ...
            email = EmailMultiAlternatives(
                ...
                to=[NOREPLY_EMAIL],
                bcc=can_help,
                headers={'X-COMETA': 'proudly_generated_by_cometa', 'X-COMETA-SERVER': COPYRIGHT, 'X-COMETA-VERSION': str(version), 'X-COMETA-FEATURE': feature.feature_name, 'X-COMETA-DEPARTMENT':feature.department_name}
            )
...
def runTest(request, *args, **kwargs):
    ...
    if apps.is_installed('extensions.payments'):
        from extensions.payments.util import get_subscriptions_from_request, check_browser_access
        try:
    ...
    if apps.is_installed('extensions.payments'):
        from extensions.payments.extensions import check_enabled_budget, check_user_will_exceed_budget
...
def MigrateScreenshots(request):
    ...
    for file in os.listdir():
        ...
        screenshot = re.match(r'^'+str(SCREENSHOT_PREFIX)+'(\d+)(_([a-z0-9]{5,}))?(_style|\.png_diff)?.png$', file)
        if not screenshot:
            continue
        images_count += 1
...
class FeatureViewSet(viewsets.ModelViewSet):
    ...
    def list(self, request, *args, **kwargs):
        ...
        if superuser:
            ...
            queryset = Feature.objects.filter(department_id__in=departments)

        if 'feature_id' in kwargs:
            feature_id = kwargs['feature_id']  
            queryset = queryset.filter(feature_id=feature_id)
            queryset[0].steps = Step.objects.filter()
            data = {
...
class FolderViewset(viewsets.ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
      ...
      if 'parent_id' in data:
        if data.get('parent_id') != 0:
            folder = Folder.objects.filter(folder_id=int(data.get('parent_id')))
            if len(folder) > 0:
                payload['parent_id'] = folder[0]
                payload['department_id'] = folder[0].department.department_id
            else:
                print("Failed to get the folder....")
      if Folder.objects.create(**payload):
    ...
    def list(self, request, *args, **kwargs):
        ...
        departments = [x['department_id'] for x in request.session['user']['departments']]
        # get all features related to the user repartments
    ...
    def patch(self, request, *args, **kwargs):
        if 'folder_id' in request.data:
            ...
            if 'name' in request.data:
                updateDict['name'] = request.data['name']
            
            if 'parent_id' in request.data:
                updateDict['parent_id'] = request.data['parent_id']

            if 'department' in request.data:
                updateDict['department'] = request.data['department']
            
            folder.update(**updateDict)

            requests.post('http://cometa_socket:3001/updatedObjects/folders')
            requests.post('http://cometa_socket:3001/sendAction', json={
                'type': '[Features] Folder got renamed',
                'folder': FolderSerializer(folder[0], many=False).data,
                'exclude': [request.session['user']['user_id']]
            })
        return JsonResponse({"success": True }, status=200)
    ...
    def destroy(self, request, *args, **kwargs):
        if 'folder_id' in kwargs:
            ...
            try:
                ...
                return JsonResponse({"success": False, "error": "No folder with folder_id: " + str(folder_id) }, status=200)
            department_id = folder[0].department.department_id
            if folder.delete():
                requests.post('http://cometa_socket:3001/updatedObjects/folders')
                requests.post('http://cometa_socket:3001/sendAction', json={
...
def sendInvite(request, receiver, departments, code, customText):
    try:
        ...
        email = EmailMultiAlternatives(
            ...
            settings.EMAIL_HOST_USER,
            to=[receiver],
            headers={'X-COMETA': 'proudly_generated_by_cometa', 'X-COMETA-SERVER': COPYRIGHT, 'X-COMETA-VERSION': str(version), 'X-COMETA-DEPARTMENTS':", ".join(departments.values_list('department_name', flat=True))}
        )
        # attatch html content (body)
        email.attach_alternative(email_body, "text/html")
        # send email
